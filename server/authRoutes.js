...
router.post('/register', async (req, res) => {
  try {
    ...
    if (!email || !password) {
      return res.status(400).json(createResponse(false, 'Email and password are required', null, 400));
    }

    if (password.length < 6) {
      return res.status(400).json(createResponse(false, 'Password must be at least 6 characters', null, 400));
    }
    ...
    if (existingUsers.length > 0) {
      await connection.release();
      return res.status(409).json(createResponse(false, 'Email already registered', null, 409));
    }
...
router.get('/profile', (req, res) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
    if (err) {
      return res.status(403).json(createResponse(false, 'Invalid token', null, 403));
    }
